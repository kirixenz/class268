...
def water_color_effect(decode_array_to_img):
    water_color_effect_img = cv2.stylization(decode_array_to_img, sigma_s=60, sigma_r=0.6)
    status, output_img = cv2.imencode('.PNG', water_color_effect_img)
...
def hdr_effect(decode_array_to_img):
    hdr_effect_img = cv2.detailEnhance(decode_array_to_img, sigma_s=10, sigma_r=0.15)
